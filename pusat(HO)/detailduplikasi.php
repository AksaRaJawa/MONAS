...
?>
    ...
    $nox=1;
		while($bxz=sqlsrv_fetch_array($bsqlx))
			{
        $sttsm = sqlsrv_query($kon, "select COUNT(status_modis) AS jml from asset where nik_karyawan = '".$bxz['nik_karyawan']."' AND status_modis = '1'", $param, $option);
        $stb = sqlsrv_fetch_array($sttsm);
        $sttsmx = sqlsrv_query($kon, "select COUNT(status_modis) AS jmlx from asset where nik_karyawan = '".$bxz['nik_karyawan']."' AND status_modis = '0'", $param, $option);
      ...
      <td><?php echo $stb['jml'] ?></td>
      <td><?php echo $stbx['jmlx'] ?></td>
		</tr>
		<?php
}
