...
$SI->setCellValue('A1', 'DATA COST CENTER == SNS'); //Judul laporan
$SI->setCellValue('A3', 'No'); //Kolom No
$SI->setCellValue('B3', 'Cost Center ID'); //Kolom
$SI->setCellValue('C3', 'Person Responsible'); //Kolom
$SI->setCellValue('D3', 'Description CC'); //Kolom
$SI->setCellValue('E3', 'Nama Depo');
$SI->setCellValue('F3', 'Nama RO');
$SI->setCellValue('G3', 'Kepala Akun');
...
$strsql	= "SELECT * from cost_center ORDER BY id_cc ";
...
while ($row = sqlsrv_fetch_array($res)) {
  ...
  $SI->setCellValue("B".$baris,$row['id_cc']); //mengisi data
  $SI->setCellValue("C".$baris,$row['person_responsible']); //mengisi data
  $SI->setCellValue("D".$baris,$row['desc_cc']); //mengisi data
  $SI->setCellValue("E".$baris,$row['nama_depo']); //mengisi data
  $SI->setCellValue("F".$baris,$row['nama_ro']); //mengisi data
  $SI->setCellValue("G".$baris,$row['kepala_akun']); //mengisi data
  $baris++; //looping untuk barisnya
}

//Memberi nama sheet
$excelku->getActiveSheet()->setTitle('Data Cost Center');

$excelku->setActiveSheetIndex(0);

// untuk excel 2007 atau yang berekstensi .xlsx
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename=cost_center'.date("Y-m-d____H:i:s").'.xlsx');
