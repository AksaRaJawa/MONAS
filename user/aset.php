...
<h4><span ></span> <?=$uu['nama_depo']?> </h4>
<button  data-toggle="modal" data-target="#myModal" class="btn btn-info col-md-2"><span class="glyphicon glyphicon-pencil"></span>  Tambah Aset</button>
<form class="col-sm-2 " action="lap_aset.php" method="post">
	<div><input type="hidden" name="uu_depo" value="<?=$uu['id_depo']?>"></div>
	<div><input type="hidden" name="uu_nama_depo" value="<?=$uu['nama_depo']?>"></div>
	<div><input type="hidden" name="uu_region" value="<?=$uu['nama_region']?>"></div>
    <div ><input type="submit" value="Export Excel" class="btn btn-warning pull-left"></div>
</form>
<form  class="col-sm-2" action="sign_aset.php" method="post">
	<div><input type="hidden" name="uu_depo" value="<?=$uu['id_depo']?>"></div>
	<div><input type="hidden" name="uu_nama_depo" value="<?=$uu['nama_depo']?>"></div>
	<div><input type="hidden" name="uu_region" value="<?=$uu['nama_region']?>"></div>
    <div><input type="submit" value="Cetak Sign Off" class="btn btn-warning pull-left"></div>
</form>
<br/>
<br/>
<br/>
...
<tbody>
	<?php
		...
		$brg=sqlsrv_query($kon, "select a.*, b.nama_karyawan, c.nama_depo from asset a left join karyawan b ON a.nik_karyawan = b.nik_karyawan left join depo c ON a.id_depo = c.id_depo where a.id_depo = '".$uu['id_depo']."' AND  a.status = '1'");
	...
	while($b=sqlsrv_fetch_array($brg)){
		...
		<tr>
            ...
            <td>
				...
				<a onclick="if(confirm('Apakah anda yakin ingin melepaskan kepemilikan asset ini ??')){ location.href='lepas_aset.php?id=<?php echo $b['id_asset']; ?>' }" class="btn btn-danger">Lepas Aset</a>
				<a href='cetak_terima.php?id=<?php echo $b['id_asset']; ?>' class="btn btn-plus">Cetak Tanda Terima</a>
				...
				<div class="modal-body">
					...
					<div class="panel panel-default">
						...
						<div class="panel-collapse collapse" id="collapseOne">
                            ...
                            <div class="form-group">
							<label>No.IMEI</label>
							<input name="no_imei" type="text" class="form-control" placeholder="No.IMEI" onkeyup="validAngka(this)" required>
							...
							<div class="form-group">
							<label>Merk</label>
							<select class="form-control autocomplete" id="nama_merk" name="nama_merk" onchange="ChangesMerk()" required>
							...
							<div class="form-group">
							<label>Depo</label>
							<input type="hidden" name="id_depo" value="<?=$uu['id_depo']?>" class="form-control">
							<input type="text" name="namadepo" value="<?=$uu['nama_depo']?>" class="form-control" autocomplete="off" disabled = "disabled">
							</div>
							<div class="form-group">
							<label>Region</label>
