...
<form action="lap_perangkat_it.php" method="post">
	<div><input type="hidden" name="uu_depo" value="<?=$uu['nama_depo']?>"></div>
    <div><input type="submit" value="Export Laporan" class="btn btn-primary pull-left"></div>
</form>
<form class="col-sm-2" action="sign_perangkat.php" method="post">
...
<br/>
	...
	<?php
		$brg=sqlsrv_query($kon, "select a.*, b.nama_karyawan, b.nama_jabatan, c.desc_cc from perangkat_it a left join karyawan_perangkat_it b ON a.nik_karyawan = b.nik_karyawan LEFT JOIN cost_center c ON a.cost_center = c.id_cc where a.nama_depo = '".$uu['nama_depo']."' AND a.status = '1' ORDER BY a.no_aset ASC");
	...
	while($b=sqlsrv_fetch_array($brg)){
		...
		<tr>
			...
			<td>
				<a href=""  data-toggle="tooltip" title="<?php echo $nama; echo " == "; echo $jabatan;?>" data-placement="top"><?php echo $b['nik_karyawan'] ?></a>
			</td>
      <td>
				...
				<a onclick="if(confirm('Apakah anda sudah memasukan Gambar dan User Responsible perangkat ini ??')) { location.href='status_validasi_perangkat.php?id=<?php echo $b['no_aset'];?>&uname=<?php echo $uu['uname'];?>' }" name="status_validasi" class="btn btn-plus"><?php if($b['status_validasi']=='1'){ ?> <span class="glyphicon glyphicon-ok"></span> <?php }else if($b['status_validasi']=='0'){ echo "Validasi";}?></a>
				<a href=''  name="status_foto"><?php if($b['foto']!=''){ ?> <span class="glyphicon glyphicon-picture"></span> <?php }else if($b['foto']==''){ echo "";}?></a>
			</td>
		</tr>
						...
						<div class="panel panel-default">
							...
							<div class="panel-collapse collapse" id="collapseTwo">
								<div class="panel-body">
									...
									<select class="form-control" id="status" name="status" onchange="ChangesStatus()">
