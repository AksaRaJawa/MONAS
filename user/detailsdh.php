...
?>
				...
				<h4 class="modal-title">Data Salesman yang Sudah Pakai Modis <?=$dp['nama_depo']?>
				</div>
				<div class="modal-body">				
					<?php
						...
						$jumlah_record1=sqlsrv_query($kon, "SELECT COUNT(*) AS jumlah from karyawan a 
								...
								WHERE a.id_depo = '".$id."' AND c.butuh_modis = '1' AND a.status = '1' AND a.nik_karyawan IN (select nik_karyawan from asset) ", $param, $option);
	...
	<?php 
		...
		LEFT JOIN depo b ON a.id_depo = b.id_depo 
		LEFT JOIN jabatan c ON a.nama_jabatan = c.nama_jabatan 
		WHERE a.id_depo = '".$id."' AND c.butuh_modis = '1' AND a.status = '1' AND a.nik_karyawan IN (select nik_karyawan from asset)");
	    $nox=1;
