...
if(sqlsrv_num_rows($cek)==1){
	if($baru==$ulang){
		...
		header("location:ganti_pass_kendaraan.php?pesan=oke");
	}else{
		header("location:ganti_pass_kendaraan.php?pesan=tdksama");
	}
}else{
	header("location:ganti_pass_kendaraan.php?pesan=gagal");
}
