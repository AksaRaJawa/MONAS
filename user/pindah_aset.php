...
?>					
	<form action="pindahkan_aset.php" method="post">
					...
					<div class="panel panel-default">
						...
						<div class="panel-collapse collapse" id="collapseZero">
							...
							<option value=" --- --- --- ">- Pilih NIK -</option>
								<?php 
								$dp=sqlsrv_query($kon, "select a.*, b.nama_jabatan, b.butuh_modis, d.nama_depo from karyawan a LEFT JOIN jabatan b ON a.nama_jabatan = b.nama_jabatan LEFT JOIN depo d ON a.id_depo = d.id_depo where a.id_depo = '".$uu['id_depo']."' AND a.status = '1' AND b.status = '1' ");
								while($rw=sqlsrv_fetch_array($dp)){
									echo'<option value="'.$rw['nik_karyawan'].'---'.$rw['butuh_modis'].'---'.$rw['nama_karyawan'].'---'.$rw['nama_jabatan'].'---'.$rw['status'].'---'.$rw['id_depo'].'---'.$rw['nama_depo'].'">
							...
							<div class="form-group">
							<label>Depo</label>
							<input type="hidden" name="id_depo" value="<?=$d['id_depo']?>" class="form-control">
							<input type="hidden" name="nama_depo" value="<?=$d['nama_depo']?>" class="form-control">
							<input type="text" id="nama_depo" name="nama_region" value="<?=$d['nama_depo']?>" class="form-control" disabled = "disabled">
						    </div>
							<div class="form-group">
							<label>Region</label>
							<input type="hidden" name="nama_region" value="<?=$d['nama_region']?>" class="form-control">
							<input type="text" id="nama_region" name="nama_region" value="<?=$d['nama_region']?>" class="form-control" disabled = "disabled">
						    </div>
							</div>
						</div>
					</div>
					...
					<div class="modal-footer">											
						<input type="submit" id="sev" class="btn btn-primary" value="Simpan">
					</div>
