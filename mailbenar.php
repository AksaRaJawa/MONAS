...
$msa = sqlsrv_query($kon, "select a.no_imei,a.nama_merk,a.nama_tipe,a.nik_karyawan,a.masa_berlaku, b.nama_karyawan, b.nama_jabatan, c.nama_depo, c.nama_region, d.penerima, d.email_penerima, d.cc, d.email_cc, d.it_region, d.emai_it, d.smd, d.email_smd, d.rach, d.email_rach, d.smd_2, d.email_smd2
	from asset a 
	LEFT JOIN karyawan b ON a.nik_karyawan = b.nik_karyawan
	LEFT JOIN depo c ON a.id_depo = c.id_depo
	LEFT JOIN mail d ON c.nama_region = d.nama_region
    ...
    while($row = sqlsrv_fetch_array($msa))
	{
        ...
        if($kurang3==$today)
		{ 
			$mail->addAddress($row['email_penerima'], $row['penerima']);
			$mail->AddCC($row['email_cc'], $row['cc']);
			$mail->AddCC($row['email_it'], $row['it_region']);
			$mail->AddCC($row['email_smd'], $row['smd']);
			$mail->AddCC($row['email_rach'], $row['rach']);
			$mail->AddCC($row['email_smd2'], $row['smd_2']);
			$mail->Subject = 'Cicilan HOP Aset Modis';
