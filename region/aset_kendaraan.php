...
<!--<button onclick="document.location='lap_aset_kendaraan.php'" class="btn btn-success col-md-2"><span class="glyphicon glyphicon-paperclip"></span>&nbsp;Export ke Excel</button>-->
</div>
<br/>
<br/>
...
<tbody >
	<?php
		...
		$brg=sqlsrv_query($kon, "select a.*, b.nama_karyawan, b.nama_jabatan, b.nama_depo from kendaraan_asetkendaraan a LEFT JOIN karyawan_perangkat_it b ON a.nik_baru = b.nik_karyawan where a.nama_region_kendaraan = '".$uu['nama_region']."'");
    ...
    $status = $b['status'];
      ...
      <td>
				<a href="edit_aset_kendaraan.php?id=<?php echo $b['no_aset_kendaraan']; ?>" class="btn btn-warning">Edit</a>
        <a href="pindah_aset_kendaraan.php?id=<?php echo $b['no_aset_kendaraan']; ?>" class="btn btn-success">Pindah Profit Center</a>
        <a onclick="if(confirm('Apakah anda sudah memasukan Gambar Ter-update ??')) { location.href='status_validasi_kendaraan.php?id=<?php echo $b['no_aset_kendaraan'];?>&uname=<?php echo $uu['uname'];?>' }" name="status_validasi" class="btn btn-plus"><?php if($b['status_validasi']=='1'){ ?> <span class="glyphicon glyphicon-ok"></span> <?php }else if($b['status_validasi']=='0'){ echo "Validasi";}?></a>
				<a href=''  name="status_gambar"><?php if($b['gambar_baru']!=''){ ?> <span class="glyphicon glyphicon-picture"></span> <?php }else if($b['gambar_baru']==''){ echo "";}?></a>
			</td>
		</tr>
