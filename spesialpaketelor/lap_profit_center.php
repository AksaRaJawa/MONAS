...
$SI->setCellValue('A1', 'DATA PROFIT CENTER == SNS'); //Judul laporan
$SI->setCellValue('A3', 'No'); //Kolom No
$SI->setCellValue('B3', 'Profit Center ID'); //Kolom
$SI->setCellValue('C3', 'Nama Depo'); //Kolom
$SI->setCellValue('D3', 'Nama RO'); //Kolom
$SI->setCellValue('E3', 'Market Tipe');
$SI->setCellValue('F3', 'Plant Description');
$SI->setCellValue('G3', 'Plant');
$SI->setCellValue('H3', 'PC Induk');
...
$strsql	= "SELECT * from profit_center ORDER BY pc ";
...
while ($row = sqlsrv_fetch_array($res)) {
  ...
  $SI->setCellValue("B".$baris,$row['pc']); //mengisi data
  $SI->setCellValue("C".$baris,$row['nama_depo']); //mengisi data
  $SI->setCellValue("D".$baris,$row['nama_ro']); //mengisi data
  $SI->setCellValue("E".$baris,$row['market_tipe']); //mengisi data
  $SI->setCellValue("F".$baris,$row['plant_description']); //mengisi data
  $SI->setCellValue("G".$baris,$row['plant']); //mengisi data
  $SI->setCellValue("H".$baris,$row['pc_induk']); //mengisi data
  $baris++; //looping untuk barisnya
}

//Memberi nama sheet
$excelku->getActiveSheet()->setTitle('Data Profit Center');

$excelku->setActiveSheetIndex(0);

// untuk excel 2007 atau yang berekstensi .xlsx
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename=profit_center'.date("Y-m-d____H:i:s").'.xlsx');
