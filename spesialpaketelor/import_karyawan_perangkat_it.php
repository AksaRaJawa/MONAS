...
if(isset($_POST['submit'])){
    ...
    if(isset($_POST['drop']))
    {
	 $truncate ="TRUNCATE table karyawan_perangkat_it";
    ...
    for ($i=2; $i<=$baris; $i++)
    {
      ...
      $id_pc	  = $data->val($i, 4);
      $id_cc	  = $data->val($i, 5);
		  $nama_depo	    = strtoupper($data->val($i, 6));
		  $nama_region	  = strtoupper($data->val($i, 7));
		  $status         = '1';
		   $cek = "select * from karyawan_perangkat_it where nik_karyawan = '".$nik_karyawan."' ";
		...
		{
		  $query = "INSERT into karyawan_perangkat_it values ('$nik_karyawan','$nama_karyawan','$nama_jabatan','$id_pc','$id_cc','$nama_depo','$nama_region','$status')";
		  $hasil = sqlsrv_query($kon, $query);
		  echo "<script>window.location='karyawan_perangkat_it.php'</script>";
		}
    }
...
<div class="form-group">
			...
			<div class="col-sm-10">
            [ NIK Karyawan ] - [ Nama Karyawan ] - [ Jabatan ] - [ ID PC ] - [ ID CC ] - [ Nama Depo ] - [ Nama Region ]
            </div>
            </div>
     <div class="form-group">
     <div class="col-sm-10">
	...
	<input type="submit" class="btn btn-primary btn-blocks" value="Import" name="submit">
	 <button type="button" class="btn btn-warning btn-blocks" onclick="document.location='karyawan_perangkat_it.php'">Kembali</button>
    </div>
    </div>
</form>
</div>
